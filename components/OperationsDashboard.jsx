...
export default function OperationsDashboard({ initialSession, portalType = 'admin' }) {
  ...
  const userDepartment = initialSession?.department || '';
  ...
  const departmentCounts = useMemo(() => complaints.reduce((acc, complaint) => {
  ...
  const departments = useMemo(
    () => Object.keys(departmentCounts).sort((a, b) => a.localeCompare(b)),
    [departmentCounts]
  );
  ...
  const heading = isAdminPortal ? 'GrievEase Admin Portal' : `${userDepartment} Dashboard`;
  const introTitle = isAdminPortal ? 'How the department assignment is handled' : 'How your department queue is handled';
  const introText = isAdminPortal
    ? 'GrievEase first predicts the complaint category using the AI pipeline. Then the frontend maps that category to the responsible civic department, and this admin portal shows the live complaint queue department-wise so the assigned team can review and act on it.'
    : `This portal is scoped to ${userDepartment}. The AI pipeline predicts the category, the application maps it to the responsible department, and this dashboard only shows the complaints assigned to your team.`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-slate-900 dark:to-gray-800">
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">{heading}</h1>
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {isAdminPortal ? 'Protected admin operations dashboard' : 'Protected department operations dashboard'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <div className="hidden md:flex flex-col items-end">
              <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">{initialSession?.name}</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {isAdminPortal ? 'Central Admin' : userDepartment}
              </span>
            </div>
            <Link
              href="/track"
            ...
            <button
              ...
              disabled={loggingOut}
              className="inline-flex items-center gap-2 rounded-xl bg-gray-900 dark:bg-gray-100 px-4 py-2 text-sm font-semibold text-white dark:text-gray-900 hover:opacity-90 disabled:opacity-60 transition-all"
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 lg:p-8">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div className="max-w-3xl">
              <span className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-blue-600 dark:text-blue-400 mb-3">
                ...
                {isAdminPortal ? 'Department Handling Flow' : 'Department Operations Flow'}
              </span>
              <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-3">{introTitle}</h2>
              <p className="text-gray-600 dark:text-gray-300 leading-relaxed">{introText}</p>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-4 mt-6">
            ...
            <div className="rounded-2xl border border-emerald-100 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/20 p-5">
              <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-300 mb-2">3. Action Queue</p>
              <p className="text-sm text-gray-700 dark:text-gray-300">Authorized officers can monitor complaints, update their status, and keep tracking synchronized for citizens.</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          ...
          <MetricCard icon={Building2} label={isAdminPortal ? 'Active Departments' : 'Department Scope'} value={isAdminPortal ? departments.length : 1} accent="bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300" />
        </section>

        <section className={`grid gap-6 items-start ${isAdminPortal ? 'xl:grid-cols-[1.2fr_2fr]' : 'xl:grid-cols-[1fr_2fr]'}`}>
          <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">
                {isAdminPortal ? 'Department Queue Overview' : 'Assigned Department Overview'}
              </h3>
            </div>
                ...
                {departments.map((department) => (
                  <div key={department} className="flex items-center justify-between rounded-2xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {isAdminPortal ? 'Assigned by category-to-department logic' : 'Visible because this account belongs to the department'}
                      </p>
                    </div>
          ...
          <div className="self-start bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
            ...
            <div className={`grid gap-3 ${isAdminPortal ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
              <div className={`${isAdminPortal ? 'md:col-span-2' : 'md:col-span-1'} relative`}>
        ...
        <section className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">
                {isAdminPortal ? 'Latest Complaint Queue' : 'Department Complaint Queue'}
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {isAdminPortal ? 'Admin queue with real complaint status control' : 'Department-scoped queue with real complaint status control'}
              </p>
            </div>
